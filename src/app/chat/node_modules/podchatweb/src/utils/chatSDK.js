...
export default class ChatSDK {
  ...
  constructor(props) {
    ...
    this.onCallStreamEvents = props.onCallStreamEvents;
  ...
  _onCallStreamEvents() {
    this.chatAgent.on("callStreamEvents", (data) => {
      const {type, ...other} = data;
      this.onCallStreamEvents(other, type);
    });
  }
  ...
  @promiseDecorator
  getCallsToJoin(resolve, reject, threadIds) {
    const params = {
      threadIds
    };
  ...
  startScreenShare(resolve, reject, callId, options = {}) {
    ...
    this.chatAgent.startScreenShare(params, (result) => {
      this.chatAgent.resizeScreenShare({
        quality: 3
      });
      resolve(result)
    });
